...
public abstract class EventsController extends BaseParticipant {
    ...
    /**
     * Intercept the message and update the last call value associated
     * to the sender.
     */
    @Override
    ...
    /**
     * This defines a step by the event controller.
     *
     * If in manual mode, the set of senders associated in this step is
     * picked and the controller sends a message to these senders that
     * allow them to call a multicast.
     *
     * In the same way, crashed process can be revived.
     *
     * Correctness is enforced (it should at least) defining the
     * required process in the view set, which contains either processes
     * that must be alive or crashed in order for the step to work as
     * expected.
     */
    protected void onStep() {
         ...
         * senders are in the current view (as expected).
         */
        ...
        /*
         * SECOND CHECK: the view (defined within the view set in the
         * config) is defined and all processes within the view are
         * effectively the participants in the current view seen by the
         * controller
         */
        ...
        /*
         * retrieve actorRefs associated to their ids If a process is
         * not associated to an ActorRef then ignore the step (the
         * required process has to join the view yet).
         */
        ...
        /*
         * if all participants required in the step are in the current
         * view then actions within the step can be executed. Otherwise
         * ignore the step... some desired process is missing...
         */
        Set<ActorRef> currentParticipants = new HashSet<>(
                this.tempView.members);
        currentParticipants.addAll(this.crashedProcesses
                .getProcessesActors());
        ...
        /*
         * here we are sure that processes defined in the senders
         * section, in the views section or in the risen section are
         * either alive or have been crashed (but still are in the
         * system, so no errors).
         */

        /*
         * Here we try to manage a small issue. We can have only the
         * point of view of the controller, so when a view change occurs
         * the controller could have correctly installed the view, while
         * other participants have not (it's likely to be just a matter
         * of milliseconds). If they have not installed the view yet,
         * then they are not able to send multicasts, so orders stated
         * by the controller won't be performed.
         *
         * So, let's wait 1 second in order to let other nodes install
         * the view and **be able** to send new messages. In a real
         * system this waiting time should approximate the worst among
         * network delays.
         */
         ...
         * the case of a receiving event (like receive and crash) then
         * allow both.
         */
        ActorRef tmpSender;
